...
public class Translator {
    ...
    public void bexpr(int bexpr_true, int bexpr_false){
        int bexpr1_true, bexpr1_false, bexpr2_true, bexpr2_false;
        switch(look.tag){
            case Tag.RELOP:
            ...
            case Tag.AND:
                match(Tag.AND);
                match('(');
                bexpr1_true = code.newLabel();
                bexpr1_false = bexpr_false;
                bexpr(bexpr1_true, bexpr1_false);
                match(')');
                match('(');
                bexpr2_true = bexpr_true;
                bexpr2_false = bexpr_false;
                code.emitLabel(bexpr1_true);
                bexpr(bexpr2_true, bexpr2_false);
                match(')');
                break;
            case Tag.OR:
                match(Tag.OR);
                match('(');
                bexpr1_true = bexpr_true;
                bexpr1_false = code.newLabel();
                bexpr(bexpr1_true, bexpr1_false);
                match(')');
                match('(');
                bexpr2_true = bexpr_true;
                bexpr2_false = bexpr_false;
                code.emitLabel(bexpr1_false);
                bexpr(bexpr2_true, bexpr2_false);
                match(')');
                break;
            case '!':
                match('!');
                bexpr1_true = bexpr_false;
                bexpr1_false = bexpr_true;
                bexpr(bexpr1_true, bexpr1_false);
                break;
            default:
